...
public class UserService : ServiceBase
{
    ...
    public async Task<PaginationItemsResponse<UserItemResponse>> GetUserItemsAsync(GetUserItemsRequest request)
    {
        ...
        return await Task.FromResult(new PaginationItemsResponse<UserItemResponse>(UserItems.Count, 1, users));
    }

    public async Task<UserDetailResponse> GetUserDetailAsync(Guid id)
    {
        return await Task.FromResult(UserDetailResponse.Default);
    }

    public async Task AddUserAsync(AddUserRequest request)
    {
        UserItems.Add(new UserItemResponse(Guid.NewGuid(), request.Name, request.DisplayName, request.Avatar, request.IDCard, request.PhoneNumber, request.CompanyName, request.Enabled, request.PhoneNumber, request.Email, DateTime.Now));
    ...
    public async Task UpdateUserAsync(UpdateUserRequest request)
    {
        ...
        UserItems.Add(new UserItemResponse(request.UserId, request.Name, request.DisplayName, request.Avatar, oldData.IDCard, oldData.PhoneNumber, request.CompanyName, request.Enabled, oldData.PhoneNumber, oldData.Email, oldData.CreationTime));
        await Task.CompletedTask;
    }
