...
{
    public AddUserCommandValidator(PhoneNumberValidator phoneValidator, IPasswordRuleProvider passwordRuleProvider)
    {
        ...
        WhenNotEmpty(command => command.User.Password, r => r.PasswordRule(passwordRuleProvider, command => command.User.ClientId));
