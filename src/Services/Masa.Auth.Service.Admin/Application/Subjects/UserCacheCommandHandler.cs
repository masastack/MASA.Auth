...
public class UserCacheCommandHandler
{
    ...
    async Task SetUserCacheAsync(Guid userId)
    {
        ...
        {
            var userModel = user.Adapt<UserModel>();
            userModel.Roles = user.Roles.Where(e => e.IsDeleted == false).Adapt<List<RoleModel>>();
            userModel.Permissions = user.Permissions.Where(e => e.IsDeleted == false).Adapt<List<SubjectPermissionRelationModel>>();
