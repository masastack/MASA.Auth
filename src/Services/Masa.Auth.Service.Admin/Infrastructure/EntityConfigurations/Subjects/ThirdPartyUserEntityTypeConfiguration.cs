...
public class ThirdPartyUserEntityTypeConfiguration : IEntityTypeConfiguration<ThirdPartyUser>
{
    public void Configure(EntityTypeBuilder<ThirdPartyUser> builder)
    {
        ...
        builder.HasOne(tpu => tpu.CreateUser).WithMany().HasForeignKey(tpu => tpu.Creator).OnDelete(DeleteBehavior.ClientSetNull);
        builder.HasOne(tpu => tpu.ModifyUser).WithMany().HasForeignKey(tpu => tpu.Modifier).OnDelete(DeleteBehavior.ClientSetNull);
    }
}
