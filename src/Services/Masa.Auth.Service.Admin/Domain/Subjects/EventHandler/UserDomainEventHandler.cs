...
public class UserDomainEventHandler
{
    ...
    {
        var response = await _autoCompleteClient.SetAsync<UserSelectDto, Guid>(
            userEvent.Users.Select(user => new UserSelectDto(user.Id, user.Name, user.Account, user.PhoneNumber, user.Email, user.Avatar))
        );
    }
